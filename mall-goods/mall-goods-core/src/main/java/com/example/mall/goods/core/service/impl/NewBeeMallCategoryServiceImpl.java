...
import com.example.mall.goods.api.service.NewBeeMallCategoryService;
import com.example.mall.goods.api.vo.NewBeeMallIndexCategoryVO;
import com.example.mall.goods.api.vo.SearchPageCategoryVO;
import com.example.mall.goods.api.vo.SecondLevelCategoryVO;
import com.example.mall.goods.api.vo.ThirdLevelCategoryVO;
import com.example.mall.goods.core.dao.GoodsCategoryMapper;
...
public class NewBeeMallCategoryServiceImpl implements NewBeeMallCategoryService {
    ...
    public String updateGoodsCategory(GoodsCategory goodsCategory) {
        ...
        if (goodsCategoryMapper.updateByPrimaryKeySelective(goodsCategory) > 0) {
            redisUtil.del("category::id-"+goodsCategory.getCategoryId());
            return ServiceResultEnum.SUCCESS.getResult();
        }
        return ServiceResultEnum.DB_ERROR.getResult();
    }
    ...
    public GoodsCategory getGoodsCategoryById(Long id) {
        String value = redisUtil.get("category::id-" + id);
        if (StringUtils.isEmpty(value)) {
            GoodsCategory category = goodsCategoryMapper.selectByPrimaryKey(id);
            redisUtil.set("category::id-" + id, JSON.toJSONString(category));
            return category;
        }
        GoodsCategory category = JSON.parseObject(value, GoodsCategory.class);
        return category;
    }
